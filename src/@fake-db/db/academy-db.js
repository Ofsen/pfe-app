...
const platsDessertsDB = {
	courses: [
		{
			id: '15459251a6d6b397565',
			nom: 'Basics of Angular',
			category: 'plats',
			prix: 200,
			description: 'Commits that need to be pushed lorem ipsum dolor sit amet, consectetur adipiscing elit.',
			ingredients: [
				{ id_ingr: '694', qte: 2, nom: 'Tomates' },
				{ id_ingr: '6954', qte: 1, nom: 'Pommes de terre' },
				{ id_ingr: '6954', qte: 1, nom: 'Pommes de terre' },
				{ id_ingr: '6954', qte: 1, nom: 'Pommes de terre' },
			],
		},
...
mock.onGet('/api/academy-app/courses').reply(() => {
	return [200, platsDessertsDB.courses.map((_course) => _.omit(_course, ['steps']))];
});

mock.onGet('/api/academy-app/course').reply((request) => {
	const { courseId } = request.params;
	const response = _.find(platsDessertsDB.courses, { id: courseId });
	return [200, response];
});
...
mock.onPost('/api/academy-app/course/save').reply((request) => {
	...
	platsDessertsDB.courses = platsDessertsDB.courses.map((_course) => {
	...
	if (!course) {
		course = data;
		platsDessertsDB.courses = [...platsDessertsDB.courses, course];
	}
